...
const formatNumber = (val: number): string =>
  val.toLocaleString("pl-PL").replace(/,/g, " ");

const parseNumber = (val: string): number =>
  Number(val.replace(/\s+/g, "")) || 0;

const formattedMin = computed(() =>
  localMin.value !== "" ? formatNumber(localMin.value) : "",
);
const formattedMax = computed(() =>
  localMax.value !== "" ? formatNumber(localMax.value) : "",
);
...
<template>
  <div class="d-flex align-items-center gap-2">
    <label v-if="label" class="form-label text-light-txt-color fs-sm mb-0">{{
      label
    }}</label>
    <div class="d-flex align-items-center gap-2">
      <input
